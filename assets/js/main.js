    ...
    $(document).ready(function () {
        ...
        function updateTotals() {
            ...
            $('tbody tr').each(function () {
                const priceEl = $(this).find('.unit-price');
                const qtyEl = $(this).find('.quantity-input');

                // فقط ردیف‌هایی که محصول واقعی دارند
                if (priceEl.length && qtyEl.length) {
                    let qty = Math.max(1, parseInt(qtyEl.val()) || 1);
                    let price = parseInt(priceEl.data('price')) || 0;

                    qtyEl.val(qty);
                    $(this).find('.subtotal-amount').text((price * qty).toLocaleString() + ' تومان');
                    total += price * qty;
                }
            });
            ...
            // اگر هیچ محصولی نبود
            if (total === 0) {
                $('.cart-totals .total-amount').text('0 تومان');
            } else {
                $('.cart-totals .total-amount').text(total.toLocaleString() + ' تومان');
            }

            // دکمه ادامه پرداخت
            const hasZero = $('tbody .quantity-input').filter(function () {
                return parseInt(this.value) < 1;
            }).length > 0;
            $('.default-btn[href="checkout/"]').prop('disabled', hasZero).css('pointer-events', hasZero ? 'none' : 'auto');
        }

        function postQuantity(id, type, qty) {
            $.post('ajax/add_to_cart.php', {action: 'update_quantity', id: id, type: type, quantity: qty});
        }
        ...
        $('.minus-btn, .plus-btn').click(function () {
            ...
            postQuantity(row.data('id'), row.data('type'), qty);
            updateTotals();
        });
        ...
        $('.remove-item').click(function () {
            ...
            if (!confirm('آیا می‌خواهید این محصول را حذف کنید؟')) return;

            $.post('ajax/add_to_cart.php', {action: 'remove_item', id: id, type: type}, function (data) {
                if (data.success) {
                    row.remove();
                    updateTotals();
                    window.location.reload();
                    if ($('tbody tr').length === 0) {
                        $('tbody').html('<tr><td colspan="6" class="text-center">سبد خرید شما خالی است.</td></tr>');
                    }
                } else {
