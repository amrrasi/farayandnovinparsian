...
function buildMenuOptions($menus, $parent_id = 0, $level = 0, $selected = 0) {
    ...
    foreach ($menus as $menu) {
        if ($menu['parent_id'] == $parent_id) {
            $prefix = str_repeat('&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp-- ', $level);
            $isSelected = ($menu['id'] == $selected) ? 'selected' : '';
            $html .= '<option value="' . htmlspecialchars($menu['id']) . '" ' . $isSelected . '>' . $prefix . htmlspecialchars($menu['name']) . '</option>';
            $html .= buildMenuOptions($menus, $menu['id'], $level + 1, $selected);
        }
    }
    return $html;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Form']) && $_POST['Form'] === "Submitted") {
                                ...
                                <form method="post" action="">
                                    ...
                                    <div class="form-group mb-4">وضعیت:
                                        <label class="radio-inline mr-3">
                                            <input type="radio" name="active" value="1" required> فعال
                                        </label>
                                        <label class="radio-inline mr-3">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-9">
                                                <select name="parent_id" class="form-control input-default">
                                                    ...
                                                    <?php echo buildMenuOptions($menus, 0, 0, $packageMenu); ?>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group col-lg-9 col-sm-12 mb-4">
                                        <div class="row">
                                            ...
                                            <div class="col-3">
                                                <input type="number" name="myorder" class="form-control input-default"
                                                       min="0" value="0">
                                            </div>
                                        </div>
                                    </div>
                                    <button type="submit" class="btn light btn-primary">ثبت منو</button>
