...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        header("Location: menu_list.php?deleted=1");
        exit;
    } else {
        $error = true;
...
function renderTableRows($mysqli, &$i, $parent_id = 0, $level = 0) {
    $stmt = $mysqli->prepare("SELECT id, name, active, myorder 
                              FROM menu 
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        <tr>
            ...
            <td>
                ...
                <a onclick="return del_confirm();" href="menu_list.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger">حذف</a>
            </td>
        </tr>
        <?php
