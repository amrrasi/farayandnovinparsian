/*!
 * persianFullCalendar v0.1.0
 * https://github.com/MostafaTech/persianFullCalendar/
 * Copyright (c) 2014 Mostafa Rowghanian. All rights reserved.
 *
 * Based on:
...
(function () {
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            self.calendar = self.el.addClass('pdp-' + options.theme);
        ...
        persianDatepicker.prototype = {
            ...
            header: function () {
                var self = this;
                _monthYear = $('<div class="yearMonthSelect" />');
                ...
                var _monthText = $('<span/>')
                        ...
                        .click(function (e) {
                            e.stopPropagation();
                            var _height = String(self.calendar.height() - 31) + 'px';
                            _yearSelect.css({ display: 'none' });
                            _monthSelect.css({ display: 'inline-block', height: _height });
                        });
                var _yearText = $('<span/>')
                        ...
                        .click(function (e) {
                            ...
                            _yearSelect.css({ display: 'inline-block', height: _height });
                    ...
                    for (var col = 0; col < 7; col++, cellIndex++) {
                        if (cellIndex < _start || cellIndex - _start + 1 > _end) {
                            ...
                            if (self.options.dates) {
                                for (di = 0; di < self.options.dates.length; di++) {
                                    var _date = self.persianDate.parse(self.options.dates[di]);
                                    if (_date.year == _dt.year && _date.month == _dt.month && _date.date == _dt.date) {
                                        _markers += '<i class="glyphicon glyphicon-pushpin"></i>';
                                    }
                                }
                            }
                            $('<div class="markers" />').html(_markers).appendTo(_cell);
                        }
            ...
            showDate: function (el, v) {
                ...
                {
                    var value = self.persianDate.parse(v).toString(self.options.formatDate);
