...
$stmt = $mysqli->prepare("SELECT * FROM menu WHERE id = ? AND deleted = 0");
...
$menus = [];
$stmt = $mysqli->prepare("SELECT id, name, parent_id FROM menu WHERE deleted = 0 AND id != ? ORDER BY myorder ASC, id ASC");
$stmt->bind_param("i", $id);
$stmt->execute();
$res = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['Form'] === "Submitted") {
    ...
    if ($name === '') {
        ...
        $stmt = $mysqli->prepare("UPDATE menu SET name = ?, active = ?, myorder = ?, parent_id = ? WHERE id = ?");
        if ($stmt) {
            $stmt->bind_param("siiii", $name,$active, $myorder, $parent_id, $id);
            if ($stmt->execute()) {
                $text = "منوی صفحه با موفقیت بروزرسانی شد";
                $menu['name'] = $name;
                $menu['active'] = $active;
                        ...
                        <div class="card-body">
                            <div class="basic-form">
                                <form method="post" action="">
                                    ...
                                    <div class="form-group col-lg-9 col-sm-12 mb-4">
                                        <div class="row">
                                            <div class="col-3"><label>نام منوی صفحه </label></div>
                                            <div class="col-9">
